 ...
 */
function spark_multipurpose_update_alpha_value_on_color_control (alpha, $control, $alphaSlider, update_slider) {
	...
	if (update_slider) {
		spark_multipurpose_update_alpha_value_on_alpha_slider(alpha, $alphaSlider);
	}
 ...
 */
function spark_multipurpose_update_alpha_value_on_alpha_slider (alpha, $alphaSlider) {
 ...
 */
function spark_multipurpose_get_alpha_value_from_color (value) {
	var alphaVal;
	...
	$(document).ready(
		function () {
			...
			function refresh_group_values (control_wrap) {
				...
				control_wrap.next('.spark-multipurpose-group-collection').val(JSON.stringify(valueToPush)).trigger('change');
			}
			...
			customize_theme_controls.on(
				...
				function () {
					...
					group_field_control.find('.group-fields').trigger('spark_multipurpose_group_slide_toggle');
				}
			);
			customize_theme_controls.on(
				'click',
			...
			function spark_multipurpose_group_field_border_style_control (group_field) {
			...
			function spark_multipurpose_group_field_check_overlay_color_control (group_field) {
			...
			customize_theme_controls.on(
				...
				function () {
					spark_multipurpose_group_field_border_style_control($(this))
				}
			);
			...
			customize_theme_controls.on(
				...
				function () {
					spark_multipurpose_group_field_check_overlay_color_control($(this))
				}
			);
			...
			customize_theme_controls.on(
				'click',
				'.spark-multipurpose-image-upload',
				function (e) {
			...
			customize_theme_controls.on(
				'click',
				'.spark-multipurpose-image-remove',
				function (e) {
	...
	wrap.find('.swp-alpha-color-control').each(
		function () {
			...
			colorPickerOptions = {
				...
				change: _.throttle(
					function (event, ui) { // For Customizer
						...
						if (defaultColor == value) {
							alpha = spark_multipurpose_get_alpha_value_from_color(value);
				...
				palettes: sparkMultipurposeLocalize.colorPalettes // Use the passed in palette.
			};
			...
			alphaVal = spark_multipurpose_get_alpha_value_from_color(startingColor);
			...
			$container.find('.min-click-zone').on(
				'click',
				function () {
					spark_multipurpose_update_alpha_value_on_color_control(0, $control, $alphaSlider, true);
				}
			);
			$container.find('.max-click-zone').on(
				'click',
				function () {
					spark_multipurpose_update_alpha_value_on_color_control(100, $control, $alphaSlider, true);
				}
			);
			...
			$container.find('.iris-palette').on(
				...
				function () {
					...
					alpha = spark_multipurpose_get_alpha_value_from_color(color);
			...
			$container.find('.button.wp-picker-clear').on(
				...
				function () {
					...
					spark_multipurpose_update_alpha_value_on_alpha_slider(100, $alphaSlider);
				}
			);
			...
			$container.find('.button.wp-picker-default').on(
				'click',
				function () {
					var alpha = spark_multipurpose_get_alpha_value_from_color(defaultColor);
			...
			$control.on(
				...
				function () {
					var value = $(this).val();
					var alpha = spark_multipurpose_get_alpha_value_from_color(value);
			...
			$alphaSlider.slider().on(
				...
				function (event, ui) {
					...
					spark_multipurpose_update_alpha_value_on_color_control(alpha, $control, $alphaSlider, false);
